...
function getpositions_req(dept_id,bunit_id,position_id,job_id)
{
    ...
    if(job_val != '')
    {
        $.post(base_url+"/default/requisition/getpositions",{bunitid:bunit_val,dept_id:dept_val,job_id:job_val},function(data){
            ...
            $('#s2id_'+position_id).find('a.select2-choice').find('span').html('Select Grading');
            var opt_len = $('#'+position_id).find('option').length;
            if(opt_len == 1)
            {
                $("#errors-"+position_id).remove();
			  $('#'+position_id).after("<span class='errors' id='errors-"+position_id+"'>Gradings are not configured yet.</span>");
            }
        },'json');
    }
    else 
    {
        $('#'+position_id).find('option').remove();
        $('#s2id_'+position_id).find('a.select2-choice').find('span').html('Select Grading');
        $('#'+position_id).html("<option value=''>Select Grading</option>");
    }
}
...
{    
    ...
    $("#"+eleId).find('option').remove();
    $("#"+eleId).html("<option value='' label='select a Superior Name'>Select a Superior Name</option>");
    $('#s2id_'+eleId).find('a.select2-choice').find('span').html('Select a Superior Name');
    if(Url != "" && empRole != "")
    {
        $.ajax({
            url:Url,   				
				        ...
				        else if(response != '' && response != 'null' && $.trim(response) != 'nomanagers')
						  {
								if(con == "req")
								{
									if($("#errors-"+eleId).is(':visible'))
										$("#errors-"+eleId).hide();						
									$('#'+eleId).trigger('change');
									$('#s2id_'+eleId).find('span').html('Select a Superior Name');
									$("#loader").remove();
									$("#"+eleId).html(response);
								}
								else
...
function displayPositions(ele,eleId,con)
{
  ...
  var params = '';
	...
	if(id !='')
	{
				        ...
				        if($.trim(response) == 'nopositions')
						 {
						    $("#loader").remove();
						    $("#errors-"+eleId).show();					
						    $("#errors-"+eleId).html("Gradings are not configured for this job title.");
							$("#"+eleId).find('option').remove();
	                        $("#"+eleId).prepend("<option value='' label='select position'>Select Grading</option>");
							$('#s2id_'+eleId).find('span').html('Select Grading');		 
						 }
        ...
        {
            $('#'+eleId).find('option').remove();       
            $('#'+eleId).html("<option value='' label='select position'>Select Grading</option>");
            $('#s2id_'+eleId).find('a.select2-choice').find('span').html('Select Grading');
        }
	...
	if(id !='')
	{
		$.ajax({				
				...
				success : function(response)
				{
					...
					var defOption = "<option value=''>Select Grading</option>";		
					$('#s2id_position_id .select2-choice span').html('Select Grading');
					$("#position_id").find('option').remove();
