...
//            exit;
					...
					$employeeData = $usersModel->getUserDetailsByIDandFlag($id);                    
		 			...
		 			else
		 			{
		 				...
		 				if(!empty($empdata))
		 				{
                            ...
                            $cnamemodel = new Default_Model_Language();
		 					...
		 					if(!empty($data))
		 					{
                                ...
                                //change
		 						...
		 						if(isset($data[0]['cname']) && $data[0]['cname'] !='')
		 						{
		 							$singlelanguageArr = $cnamemodel->getLanguageDataByID($data[0]['cname']);
		 							if(!empty($singlelanguageArr))
		 							{
										$emppersonaldetailsform->cname->addMultiOption($singlelanguageArr[0]['id'],$singlelanguageArr[0]['languagename']);
										$data[0]['cname']=$singlelanguageArr[0]['languagename'];
		 							}
									else
									{
										$data[0]['cname']="";
	...
	public function editAction()
	{
		...
		if(defined('EMPTABCONFIGS'))
		{
			...
			if(in_array('emppersonaldetails',$empOrganizationTabs)){
				...
				$languagepermission = sapp_Global::_checkprivileges(LANGUAGE,$loginUserGroup,$loginUserRole,'add');
                				$languagepermissio = sapp_Global::_checkprivileges(LANGUAGE,$loginUserGroup,$loginUserRole,'add');
					...
					if($id && is_numeric($id) && $id>0 && $id!=$loginUserId)
					{
						...
						else
						{
							...
							if(!empty($empdata))
							{
                                ...
                                $languageArr1 = $cnamemodel->gettotalLanguageData();
								if(!empty($languageArr1))
								{
									foreach ($languageArr1 as $languageres1){
										$emppersonaldetailsform->cname->addMultiOption($languageres1['id'],$languageres1['languagename']);
								...
								if(!empty($data))
								{
									...
									*/
                                    ...
                                    $emppersonaldetailsform->setDefault('cname',$data[0]['cname']);
								}
        ...
        $this->view->languagepermissio = $languagepermissio;
		...
		if($auth->hasIdentity()){
			$loginUserId = $auth->getStorage()->read()->id;
		        ...
		        if(!empty($new_post_values))
		        {
                    ...
                    $mname= $this->_request->getParam('mname');
                    $genderid = $this->_request->getParam('genderid');
                    
					$maritalstatusid = $this->_request->getParam('maritalstatusid');
                    
                    //change
                    $wname= $this->_request->getParam('wname');
		            ...
		            $bp = $this->_request->getParam('bp');
		//$idcn,$fmcn,$npcn,$bpjs,$bpjsn
                    
                      $idcn= $this->_request->getParam('idcn');
                    $fmcn= $this->_request->getParam('fmcn');
                      $npcn= $this->_request->getParam('npcn');
                    $bpjs= $this->_request->getParam('bpjs');
                      $bpjsn= $this->_request->getParam('bpjsn');
