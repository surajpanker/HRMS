...
class Default_PositionsController extends Zend_Controller_Action
{
	...
	public function save($positionsform)
	{
		...
		if($positionsform->isValid($this->_request->getPost())){
			...
			{
				$tableid = $id;
				$this->_helper->getHelper("FlashMessenger")->addMessage(array("success"=>"Grading updated successfully."));
			}
			else
			{
				$tableid = $Id;
				$this->_helper->getHelper("FlashMessenger")->addMessage(array("success"=>"Grading added successfully."));
			}
			$menuID = POSITIONS;
			$result = sapp_Global::logManager($menuID,$actionflag,$loginUserId,$tableid);
	...
	public function deleteAction()
	{
		...
		if($id)
		{
			...
			if($Id == 'update')
			{
				sapp_Global::send_configuration_mail("Gradings", $position_data[0]['positionname']);
				$menuID = POSITIONS;
				$result = sapp_Global::logManager($menuID,$actionflag,$loginUserId,$id);
				$messages['message'] = 'Grading deleted successfully.';
				$messages['msgtype'] = 'success';
			}
			else
			{	 $messages['message'] = 'Grading cannot be deleted.';
