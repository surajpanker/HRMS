...
class Default_MyemployeesController extends Zend_Controller_Action
{
		...
		if($id)	
		{
			...
			else if(!empty($data))
			{
				...
				{ 			                    
					...
					{ 			
						$employeeform->reporting_manager->addMultiOption('','Select a Superior Name');
				...
				{ 			
						$employeeform->position_id->addMultiOption('','Select a Grading');
						...
						else
						{
							...
							{ 
								...
								if(!empty($data))
								{
									...
									if($data[0]['emergency_country']!='' && $data[0]['emergency_state'] !='')
									{
										
										$countriesArr = $countriesModel->getCountryCode($data[0]['emergency_country']);
										if(sizeof($countriesArr)>0)
										{
											$empcommdetailsform->emergency_country->addMultiOption('','Select Country');
											foreach ($countriesArr as $countrieslistres)
											{
												$empcommdetailsform->emergency_country->addMultiOption($countrieslistres['id'],$countrieslistres['country_name']);
											}
										}
										
										$stateEmerlistArr = $statesmodel->getStatesList($data[0]['emergency_country']); 
										if(sizeof($stateEmerlistArr)>0)
										{
											$empcommdetailsform->emergency_state->addMultiOption('','Select State');
											foreach($stateEmerlistArr as $stateEmerlistres)	
											 {
												$empcommdetailsform->emergency_state->addMultiOption($stateEmerlistres['id'].'!@#'.$stateEmerlistres['state_name'],$stateEmerlistres['state_name']); 
											 }   						 
										}
										$emerStateNameArr = $statesmodel->getStateName($data[0]['emergency_state']);
										
									}
									if($data[0]['emergency_country']!='' && $data[0]['emergency_state'] !='' && $data[0]['emergency_city']!='')
									{
										$cityEmerlistArr = $citiesmodel->getCitiesList($data[0]['emergency_state']); 
										
										if(sizeof($cityEmerlistArr)>0)
										{
											$empcommdetailsform->emergency_city->addMultiOption('','Select State');
											foreach($cityEmerlistArr as $cityEmerlistres)	
											 {
												$empcommdetailsform->emergency_city->addMultiOption($cityEmerlistres['id'].'!@#'.$cityEmerlistres['city_name'],$cityEmerlistres['city_name']); 
											 }   						 
										}
										$emerCityNameArr = $citiesmodel->getCityName($data[0]['emergency_city']);
									}
									...
									if($data[0]['emergency_country'] != '')   
									   $empcommdetailsform->setDefault('emergency_country',$data[0]['emergency_country']);
									if($data[0]['emergency_state'] !='')
									   $empcommdetailsform->setDefault('emergency_state',$emerStateNameArr[0]['id'].'!@#'.$emerStateNameArr[0]['statename']);
									if($data[0]['emergency_city'] != '')   
									   $empcommdetailsform->setDefault('emergency_city',$emerCityNameArr[0]['id'].'!@#'.$emerCityNameArr[0]['cityname']);
							    }
									...
									if(!empty($data[0]['emergency_country']))
									 {
									   $countryname = $countriesModel->getCountryCode($data[0]['emergency_country']);
										  if(!empty($countryname))
										  {
										   $data[0]['emergency_country'] = $countryname[0]['country_name'];
										  }
										  else
										  {
											  $data[0]['emergency_country'] = "";
										  }
									 }
									if(!empty($data[0]['emergency_state']))
									 {
										$statename = $statesmodel->getStateName($data[0]['emergency_state']);
										  if(!empty($statename))
										  {
										   $data[0]['emergency_state'] = $statename[0]['statename'];
										  }
										  else
										  {
											  $data[0]['emergency_state'] = ""; 
										  }
									 }
									if(!empty($data[0]['emergency_city'])) 
									{
										$cityname = $citiesmodel->getCityName($data[0]['emergency_city']);
										 if(!empty($cityname))
										 {
										  $data[0]['emergency_city'] = $cityname[0]['cityname'];
										 }
										 else
										 {
											 $data[0]['emergency_city'] = "";
										 }
									}
							$this->view->controllername = $objName;
							$this->view->data = $data;
					...
					{  
						...
						if(!empty($empdata))
						{
							...
							$tableFields = array('action'=>'Action','educationlevel'=>'Education Level',
										 'institution_name'=>'Institution Name','course'=>'Major',
										 'from_date'=>'From',"to_date"=>"To","percentage"=>"GPA");
	...
	public function addAction()
	{
		...
		else
		{
			...
			$msgarray['position_id'] = 'Gradings are not configured yet.';
			$emptyFlag++;
		}
	...
	public function save($employeeform)
	{
		...
		else
		{
			...
			if(isset($jobtitle_id) && $jobtitle_id != 0 && $jobtitle_id != '')
			{
				...
				$employeeform->position_id->addMultiOption('','Select Grading');
				foreach($positionlistArr as $positionlistRes)
				{
		...
		if($auth->hasIdentity()){
			$loginUserId = $auth->getStorage()->read()->id;
				...
				if(!empty($expiry_dateArr))
				{
					...
					{
							if($expiry_dateArr[$j] == '' && $mandatorydocArr[$i] == 1)
							{
                            ...
                            if(!empty($empdata))
                            {
                                ...
                                if(sizeof($countrieslistArr)>0)
                                {
                                    ...
                                    foreach ($countrieslistArr as $countrieslistres)
                                    {
                                        ...
                                        $empcommdetailsform->emergency_country->addMultiOption($countrieslistres['id'],utf8_encode($countrieslistres['country_name']));
                                    }
                                }
                                else
                                {
                                    ...
                                    $msgarray['emergency_country'] = 'Countries are not configured yet.';
                                }
                                $data = $empcommdetailsModal->getsingleEmpCommDetailsData($id);
                                if(!empty($data))
                                {
                                    ...
                                    $emergency_country = $data[0]['emergency_country'];
                                    if(isset($_POST['emergency_country']))
                                    {
                                        $emergency_country = $_POST['emergency_country'];
                                    }
                                    $emergency_state = $data[0]['emergency_state'];
                                    if(isset($_POST['emergency_state']))
                                    {
                                        $emergency_state = $_POST['emergency_state'];
                                    }
                                    $emergency_city = $data[0]['emergency_city'];
                                    ...
                                    {
                                        $stateEmerlistArr = $statesmodel->getStatesList($emergency_country);
                                        if(sizeof($stateEmerlistArr)>0)
                                        {
                                            $empcommdetailsform->emergency_state->addMultiOption('','Select State');
                                            foreach($stateEmerlistArr as $stateEmerlistres)
                                            {
                                                $empcommdetailsform->emergency_state->addMultiOption($stateEmerlistres['id'],$stateEmerlistres['state_name']);
                                            }
                                        }
                                    }
                                    if($emergency_state != '')
                                    {
                                        $cityEmerlistArr = $citiesmodel->getCitiesList($emergency_state);

                                        if(sizeof($cityEmerlistArr)>0)
                                        {
                                            $empcommdetailsform->emergency_city->addMultiOption('','Select City');
                                            foreach($cityEmerlistArr as $cityEmerlistres)
                                            {
                                                $empcommdetailsform->emergency_city->addMultiOption($cityEmerlistres['id'],$cityEmerlistres['city_name']);
                                            }
                                        }
                                    }
                                    ...
                                    if($data[0]['emergency_country'] != '')
                                        $empcommdetailsform->setDefault('emergency_country',$emergency_country);
                                    if($data[0]['emergency_state'] !='')
                                        $empcommdetailsform->setDefault('emergency_state',$emergency_state);
                                    if($data[0]['emergency_city'] != '')
                                        $empcommdetailsform->setDefault('emergency_city',$emergency_city);
                                }
	...
	public function comsave($empcommdetailsform,$userid)
	{
		...
		$emergency_country = $this->_request->getParam('emergency_country');
		$emergency_stateparam = $this->_request->getParam('emergency_state');
		$emergency_stateArr = explode("!@#",$this->_request->getParam('emergency_state'));
		$emergency_state = $emergency_stateArr[0];
		$emergency_cityparam = $this->_request->getParam('emergency_city');
		$emergency_cityArr = explode("!@#",$this->_request->getParam('emergency_city'));
		$emergency_city = $emergency_cityArr[0];
		...
		if($empcommdetailsform->isValid($this->_request->getPost())){
           ...
           if(!empty($new_post_values))
           {
			...
			$emergency_relationship = $this->_request->getParam('emergency_relationship');

			$emergency_streetaddress = $this->_request->getParam('emergency_streetaddress');
			$emergency_pincode = $this->_request->getParam('emergency_pincode');
			...
			$data = array('user_id'=>$user_id,
								 ...
								 'emergency_relationship'=>($emergency_relationship!=''?$emergency_relationship:NULL),

                                 'emergency_streetaddress'=>($emergency_streetaddress!=''?$emergency_streetaddress:NULL), 
                                 'emergency_country'=>($emergency_country!=''?$emergency_country:NULL),	 
				      			 'emergency_state'=>($emergency_state!=''?$emergency_state:NULL),
								 'emergency_city'=>($emergency_city!=''?$emergency_city:NULL), 								 
				      			 'emergency_pincode'=>($emergency_pincode!=''?$emergency_pincode:NULL),
		...
		}else
		{
			...
			if(isset($emergency_country) && $emergency_country != 0 && $emergency_country != '')
			{
				$statesmodel = new Default_Model_States();
				$statesmodeldata = $statesmodel->getStatesList($emergency_country);
					
				$empcommdetailsform->emergency_state->addMultiOption('','Select State');
				foreach($statesmodeldata as $res)
				$empcommdetailsform->emergency_state->addMultiOption($res['id'].'!@#'.utf8_encode($res['state_name']),utf8_encode($res['state_name']));
				if(isset($emergency_stateparam) && $emergency_stateparam != 0 && $emergency_stateparam != '')
				$empcommdetailsform->setDefault('emergency_state',$emergency_stateparam);
			}
			if(isset($emergency_stateparam) && $emergency_stateparam != 0 && $emergency_stateparam != '')
			{
				$citiesmodel = new Default_Model_Cities();
				$citiesmodeldata = $citiesmodel->getCitiesList($emergency_state);
					
				$empcommdetailsform->emergency_city->addMultiOption('','Select City');
				foreach($citiesmodeldata as $res)
				$empcommdetailsform->emergency_city->addMultiOption($res['id'].'!@#'.utf8_encode($res['city_name']),utf8_encode($res['city_name']));
				if(isset($emergency_cityparam) && $emergency_cityparam != 0 && $emergency_cityparam != '')
				$empcommdetailsform->setDefault('emergency_city',$emergency_cityparam);
			}
			return $msgarray;
		}
	}
	...
	public function employeereportAction()
	{
            ...
            if($loginuserGroup==MANAGER_GROUP || $loginuserGroup==MANAGEMENT_GROUP || $loginuserRole==SUPERADMINROLE)
            {
	            ...
	            if(count($job_data)==0)
	            {
	                ...
	                $norec_arr['position_id'] = "Gradings are not configured yet.";
	            }
