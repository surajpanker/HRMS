...
class Default_EmpjobhistoryController extends Zend_Controller_Action
{
	...
	public function addpopupAction()
	{
		...
		$reportingManagerArr = $usersModel->getSuperiorNamesListForJobHistory();
		$empjobhistoryform->reporting_manager_id->addMultiOption('','Select Superior Name');
		if(!empty($reportingManagerArr))
		{
			foreach ($reportingManagerArr as $rmres){
				$empjobhistoryform->reporting_manager_id->addMultiOption($rmres['id'],utf8_encode($rmres['name']));	
			}
		}else
		{
			$msgarray['reporting_manager_id'] = 'Superior Name are not configured yet.';
			$emptyFlag++;
		}
		$positionArr = $positionModel->getTotalPositionList();
		$empjobhistoryform->positionheld->addMultiOption('','Select Grading');
		...
		{
			$msgarray['positionheld'] = 'Gradings are not configured yet.';
			$emptyFlag++;
		}
	...
	public function viewpopupAction()
	{
		...
		if($id)
		{
			...
			if(!empty($data))
			{
				...
				$reportingManagerArr = array();
				if(is_numeric($data[0]['reporting_manager_id']))
				{
					$reportingManagerArr = $usersModel->getSuperiorNamesListForJobHistory($data[0]['reporting_manager_id']);
				}
				if(!empty($reportingManagerArr))
				{
					$empjobhistoryform->reporting_manager_id->addMultiOption($reportingManagerArr[0]['id'],$reportingManagerArr[0]['name']);
					$data[0]['reporting_manager_id']=$reportingManagerArr[0]['name'];
				}
				else
				{
					$data[0]['reporting_manager_id']="";
				}
	...
	public function editpopupAction()
	{
		...
		if($id)
		{
			...
			if(!empty($data))
			{
				...
				$empjobhistoryform->setDefault('reporting_manager_id',$data[0]['reporting_manager_id']);
	...
	public function save($empjobhistoryform,$userid)
	{
		...
		if($empjobhistoryform->isValid($this->_request->getPost()))
		{
			...
			$reporting_manager_id = $this->_request->getParam('reporting_manager_id');
