...
class Default_LeaverequestController extends Zend_Controller_Action
{
	...
	{   
	  ...
	  $auth = Zend_Auth::getInstance();
		...
		$sickleave = $this->_request->getParam('sickleave'); 
		...
		*/
		...
		if($leaverequestform->isValid($this->_request->getPost()) && $errorflag == 'true')
            ...
            //*******************************************************
                    ...
                    if ($_FILES["leave_document"]["size"] == 0 || $_FILES["leave_document"]["size"] > (2*1024*1024)) {
                        $this->_helper->json(array('error' => 'filesize'));
                    } else if(isset($_FILES["leave_document"])) {
                        $fileName = $_FILES["leave_document"]["name"];
                        ...
                        move_uploaded_file($_FILES["leave_document"]["tmp_name"],EMP_DOC_UPLOAD_PATH.$newName);
                        
                        //insert document
                        $documentData = array(
								'user_id' => $loginUserId,
								'name' => $newName,
		                        'attachments' => json_encode(array(["original_name" => $newName, "new_name" => $newName])),
		                        'createdby' => $loginUserId,
		                        'createddate'=>gmdate("Y-m-d H:i:s")
						);
                        $empDocuModel = new Default_Model_Employeedocs();
                        $recordId = $empDocuModel->SaveorUpdateEmpDocuments($documentData, '');
                        $data['document_id'] = $recordId;
                        
                    }
                    //die("end save document");
                //===end save document
