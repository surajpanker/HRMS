...
class Default_Form_Requisition extends Zend_Form
{
    public function init()
    {
        ...
        $reporting_id->setAttrib('title', 'Superior Name.');         
	    $reporting_id->setRegisterInArrayValidator(false);  
        $reporting_id->addMultiOptions(array(''=>'Select Superior Name'));
        
        $position_id = new Zend_Form_Element_Select("position_id");
        $position_id->setAttrib("class", "formDataElement");
        $position_id->setAttrib('title', 'Grading.');  
        $position_id->addMultiOptions(array(''=>'Select Grading'));
    ...
    return $example;
        ...
        $req_no_positions->setAttrib('maxLength', 1);
