...
class Default_Form_empcommunicationdetails extends Zend_Form
{
    public function init()
    {
        ...
        $emergency_relationship = new Zend_Form_Element_Text('emergency_relationship');
        $emergency_relationship->setAttrib('maxLength', 50);
        $emergency_relationship->addFilter(new Zend_Filter_StringTrim());

        $emergency_streetaddress = new Zend_Form_Element_Text('emergency_streetaddress');
        $emergency_streetaddress->setAttrib('maxLength', 100);
        $emergency_streetaddress->addFilter(new Zend_Filter_StringTrim());
    
      
        $emergency_country = new Zend_Form_Element_Select('emergency_country');
        
        $emergency_country->setAttrib('onchange', 'displayParticularState(this,"","emergency_state","")');
        $emergency_country->setRegisterInArrayValidator(false);
        $emergency_country->addMultiOption('','Select Country');

        $emergency_state = new Zend_Form_Element_Select('emergency_state');
        
        $emergency_state->setAttrib('onchange', 'displayParticularCity(this,"","emergency_city","")');
        $emergency_state->setRegisterInArrayValidator(false);
        $emergency_state->addMultiOption('','Select State');
          
        $emergency_city = new Zend_Form_Element_Select('emergency_city');
        
        $emergency_city->setRegisterInArrayValidator(false);
        $emergency_city->addMultiOption('','Select City');
    

        $emergency_pincode = new Zend_Form_Element_Text('emergency_pincode');
        $emergency_pincode->setAttrib('maxLength', 10);
        $emergency_pincode->addFilter(new Zend_Filter_StringTrim());
        $emergency_pincode->addValidators(array(array('StringLength',false,
                                  array('min' => 3,
                                      'max' => 10,
                                        'messages' => array(
                                        Zend_Validate_StringLength::TOO_LONG =>
        ...
        $emergency_pincode->addValidator("regex",true,array(
                        'pattern'=>'/^(?!0{3})[0-9a-zA-Z]+$/', 
        ...
        $this->addElements(array($id,$userid,$personalemail,$perm_streetaddress,$perm_country,$perm_state,$perm_city,$perm_pincode,$current_streetaddress,$current_country,$current_state,$current_city,$current_pincode,$address_flag,$emergency_number,$emergency_name,$emergency_relationship,$emergency_streetaddress,$emergency_country,$emergency_state,$emergency_city,$emergency_pincode,$submit));
