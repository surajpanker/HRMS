...
class Default_Model_Empjobhistory extends Zend_Db_Table_Abstract
{
	...
	public function getEmpJobHistoryData($sort, $by, $pageNo, $perPage,$searchQuery,$id)
	{
		...
		$empjobhistoryData = $this->select()
						   ...
						   ->joinLeft(array('u'=>'main_users'),'u.id=e.reporting_manager_id',array('reporting_manager_id'=>'u.userfullname'))
						   ->where($where)
    					   ->order("$by $sort") 
	...
	{		
		...
		$tableFields = array('action'=>'Action','department'=>'Department','positionheld'=>'Position Name','reporting_manager_id'=>'Superior Name','start_date'=>'From','end_date'=>'To');
