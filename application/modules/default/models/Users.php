...
class Default_Model_Users extends Zend_Db_Table_Abstract
{
	...
	public function getSuperiorNamesListForJobHistory($id='')
	{
		$db = Zend_Db_Table::getDefaultAdapter();
		$qry = "select u.id, concat(u.userfullname,if(j.jobtitlename is null,'',concat(' , ',j.jobtitlename))) as name 
				FROM main_users u 
				INNER JOIN main_employees e ON u.id = e.user_id 
		...
		$qry .= " ORDER BY u.userfullname ASC";
		$reportingManagersData = $db->query($qry);
		return $reportingManagersData->fetchAll();
	}
